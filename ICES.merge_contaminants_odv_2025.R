...
#EMD 2025 transposed data provided in https://drive.google.com/drive/u/1/folders/1eSt5zWwT52Vvz32KUtbm0SqdVyFAAwnI
####################################
...
bio_dpAll <- rbindlist(list(bio_dp1,bio_dp2,bio_dp3,bio_dp4), use.names = TRUE, fill=TRUE)

# #Compare headers with preferred import header - this is one way to check if the oprder of columns are the same before renaming (manual inspection in the end...)
...
max_lengths <- sapply(bio_dpAll, function(column) {
  # Remove NA values
...
setnames(sed_dp2, "Depth.below.seabed..m.", "DepBelowBed..m.", skip_absent=TRUE)
#Baltic
